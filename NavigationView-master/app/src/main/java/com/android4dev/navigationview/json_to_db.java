 ...
 */
public class json_to_db extends AppCompatActivity{
    SQLiteDatabase db;
    DBHelper mydb = new DBHelper(this);
    String jenis_member="",password="",npwp="",username="",no_fax_kantor="",akta_pendirian="",alamat_kantor="",email_company="",email_user="",id_member="",ijin_usaha="",jabatan_pimpinan="",jabatan_user="",jenis_kantor="",kode_pos="",kota="", merk_brand="", nama_lengkap="",nama_perusahaan="",nama_pimpinan="",no_hp_pim="",no_hp_user="",no_telp_kantor="";

    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        Login mylogin = new Login();
        Cursor rs = mydb.getDataLogin(mylogin.username);
        if (rs==null) {
            getData();
        }
    }
        ...
        StringRequest stringRequest = new StringRequest(url,
                new Response.Listener<String>() {
                    ...
                    public void onResponse(String response) {
                        showJSON(response);
                        Toast.makeText(json_to_db.this,nama_lengkap,Toast.LENGTH_LONG).show();
                    }
                },
                new Response.ErrorListener() {
                    @Override
                    public void onErrorResponse(VolleyError error) {
                        Toast.makeText(json_to_db.this,error.getMessage(),Toast.LENGTH_LONG).show();
                    }
                });
